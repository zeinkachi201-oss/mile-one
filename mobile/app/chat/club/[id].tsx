...
import { api, Message, Club } from '../../../services/api';
    ...
    <SafeAreaView style={s.container}>
      <View style={s.header}>
        ...
        <View style={s.headerAvatar}>
          <Text style={s.headerAvatarText}>{club?.name?.[0]?.toUpperCase() ?? '#'}</Text>
        </View>
        <View>
          <Text style={s.title}>{club?.name || 'Club Chat'}</Text>
          {club && <Text style={s.subtitle}>{club.member_count} members</Text>}
        </View>
      </View>
...
const s = StyleSheet.create({
  ...
  headerAvatar: { width: 36, height: 36, borderRadius: 10, backgroundColor: colors.brand, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
